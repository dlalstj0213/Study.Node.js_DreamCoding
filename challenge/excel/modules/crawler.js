...
const fs = require('fs');
const url = require(path.join(process.cwd(), 'res', 'json', 'url.json'));

const maxTotal = 500;
const selectors = {
	rank_div:
		'#content > div.section_instie_area.space_top > div > div:nth-child(2) > div.section_insite_sub > div',
...
class Crawler {
	...
	async crawl(opt, search) {
		...
		try {
			...
			//await page.waitForSelector(selectors.rank_div);
