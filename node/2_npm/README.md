- [npm](#npm)
  ...
  - [npm command](#npm-command)
    ...
    - [패키지 버전상태 조회](#패키지-버전상태-조회)
    - [패키지 업데이트](#패키지-업데이트)
- [유용한 패키지](#유용한-패키지)
  - [개발 할때만 필요한 라이브러리](#개발-할때만-필요한-라이브러리)
    - [nodemon](#nodemon)
...
```bash
$ npm i [library-name] --save-dev
```

- `--save-dev` 옵션을 추가하면 `package.json`에 `devDependencies:` 하위로 라이브러리가 추가된다.
...
```shell
$ npm outdated [library-name]
```

- 라이브러리 이름을 명시하지 않을시 전체 라이브러리 조회
- `Current` `Wanted` `Latest` `Location` 를 조회가능
...
- 라이브러리 이름을 명시하지 않을시 전체 라이브러리 업데이트
...
- `nodemon`으로 서버를 작동시 서버가 계속 동작하면서 파일이 변경될때 서버를 재시작해주기 때문에 개발할때 도움을 많이 주는 라이브러리
- 마치 hotdeploy 같은 기능을 하는것 같음

- `nodemon`를 `devDependencies:` 하위로 설치

```bash
$ npm i nodemon --save-dev
```

- 하지만 개발할때만 사용되는 라이브러리이기 때문에 설치시 `--save-dev` 옵션을 꼭 추가해야한다.
- 마지막으로 `package.json` 에서 `script:` 에 `start` 값을 `nodemon [실행시킬파일]` 을 입력해준 후 `npm start` 명령어로 `nodemon` 을 실행시켜보자
